...
def _gbrain_dirs() -> tuple[Path, ...]:
    """Directories that may hold GBrain state, highest priority first.

    `GBRAIN_HOME` names the state directory itself. Otherwise the conventional
    `.gbrain` under `$HOME`, then under `$HERMES_HOME`. No user-specific paths
    are baked in: any install is described entirely by these variables.
    """
    dirs: list[Path] = []
    raw = os.environ.get("GBRAIN_HOME", "").strip()
    if raw:
        dirs.append(Path(raw).expanduser())
    home = os.environ.get("HOME", "").strip()
    if home:
        dirs.append(Path(home).expanduser() / ".gbrain")
    hermes = os.environ.get("HERMES_HOME", "").strip()
    if hermes:
        dirs.append(Path(hermes).expanduser() / ".gbrain")
    return _dedupe(dirs)
...
def _token_paths() -> tuple[Path, ...]:
    return tuple(d / "hermes-mcp.token" for d in _gbrain_dirs())


def _env_paths() -> tuple[Path, ...]:
    return tuple(home / ".env" for home in _hermes_homes())
